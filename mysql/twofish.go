...
// Function to encrypt a string using Twofish in CBC mode with a fixed IV
func encrypt(plainText string, key []byte) (string, error) {
	...
	plainBytes := pad([]byte(plainText), blockSize)

	// Create ciphertext slice
	cipherText := make([]byte, len(plainBytes))

	// Encrypt in CBC mode using the fixed IV
...
func main() {
	...
	// Encryption key (must be 16, 24, or 32 bytes long for Twofish)
	...
	query := `SELECT L_ORDERKEY, L_PARTKEY, L_SUPPKEY, L_LINENUMBER, L_QUANTITY, 
	                 ...
	                 L_SHIPMODE, L_COMMENT FROM lineitem limit 3000000`
	...
	insertStmt := `INSERT INTO lineitem3m_enc_Twofish (
